...
try{const data=await (await fetch("/api/update-content",{cache:"no-store"})).json();window.__contentSha=data.__sha||window.__contentSha||"";hero=data.hero||{};keys.forEach(key=>g(key).value=hero[key]||"");const slides=Array.isArray(hero.slides)&&hero.slides.length?hero.slides:[hero.image||""];slides.forEach(addSlide);s("Loaded");}catch(error){addSlide();s(error.message);}
g("add").onclick=()=>addSlide();g("save").onclick=async()=>{keys.forEach(key=>hero[key]=g(key).value.trim());const slides=Array.from(document.querySelectorAll("[data-url]")).map(input=>input.value.trim()).filter(Boolean);if(!slides.length)return s("Add at least one hero photo.");hero.slides=slides;hero.image=slides[0];s("Saving…");try{const response=await fetch("/api/update-content",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({sha:window.__contentSha,hero})});if(!response.ok){const data=await response.json().catch(()=>({}));throw new Error(data.error||"Save failed.");}s("Saved to GitHub. Vercel is deploying the hero change.");}catch(error){s(error.message);}};
</script>`); 
}